...
#cantidad de instancias de backend a ejecutar
NWEB = 2

deploy:       set_access	deploy-db       deploy-web       deploy-fe
destroy:      destroy-db      destroy-web      destroy-fe
console-log:  console-log-db  console-log-web  console-log-fe

#seteamos los accesos en la DB, con password random en cada deploy completo
set_access:
	$(eval dbpass:=$(shell curl -s http://consul.cloud.um.edu.ar:8500/v1/kv/prod-d1cor/dbpass?raw))
	$(eval dbuser:=$(shell curl -s http://consul.cloud.um.edu.ar:8500/v1/kv/prod-d1cor/dbuser?raw))
	$(eval dbname:=$(shell curl -s http://consul.cloud.um.edu.ar:8500/v1/kv/prod-d1cor/dbname?raw))
	
	$(eval k:=$(shell tr -dc A-Za-z0-9_ < /dev/urandom |head -c $${1:-16}|xargs))
	@if test -z "$(dbpass)" ; then \
		curl -X PUT -d "$(k)" http://consul.cloud.um.edu.ar:8500/v1/kv/$(PREFIX)/dbpass; \
	fi
	
	@if test -z "$(dbuser)" ; then \
		curl -X PUT -d "pdns" http://consul.cloud.um.edu.ar:8500/v1/kv/$(PREFIX)/dbuser; \
	fi
	
	@if test -z "$(dbname)" ; then \
		curl -X PUT -d "pdns" http://consul.cloud.um.edu.ar:8500/v1/kv/$(PREFIX)/dbname; \
	fi
...
deploy-web:
	make do-deploy-web EXTRA="--min-count $(NWEB)"
...
do-deploy-%: %.tmp.yaml
	...
	@nova list|grep -q -w -- '$(PREFIX)-$(*)' && exit 0; \
	    set -x;sleep 2;bash ./nova-boot.sh $(PREFIX)-$(*) $(*).tmp.yaml $(EXTRA)
...
%.tmp.yaml: %.yaml
	...
	@sed -e "s/EDITAR_USUARIO/$(USER)/" -e "s/EDITAR_LP/$(LP_USER)/" -e "s/EDITAR_ENV/$(ENV)/" -e "s/NRO_WEBS/$(NWEB)/" $^ > $@
